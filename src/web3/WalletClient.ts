...
export class WalletClient extends BaseClient implements IWalletClient {
    ...
    public constructor(
        ...
        this.getWalletAccountByAddress =
            this.getWalletAccountByAddress.bind(this);
        ...
        this.removeAddressesFromWallet =
            this.removeAddressesFromWallet.bind(this);
    ...
    public async addSecretKeysToWallet(
        ...
        for (const secretKey of secretKeys) {
            const secretKeyBase58Decoded =
                WalletClient.getBytesSecretKey(secretKey);
            ...
            const publicKeyBase58Encoded: string =
                PUBLIC_KEY_PREFIX +
                base58Encode(Buffer.concat([version, publicKey]));
    ...
    public async addAccountsToWallet(
        ...
        for (const account of accounts) {
            ...
            if (
                account.publicKey &&
                account.publicKey !== publicKeyBase58Encoded
            ) {
                throw new Error(
    ...
    public static async walletGenerateNewAccount(): Promise<IAccount> {
        ...
        const secretKeyBase58Encoded: string =
            SECRET_KEY_PREFIX +
            base58Encode(Buffer.concat([version, secretKey]));
    ...
    public async signMessage(
        ...
        const signerAccount =
            this.getWalletAccountByAddress(accountSignerAddress);
        if (!signerAccount) {
            throw new Error(
    ...
    public static async walletSignMessage(
        ...
        if (signer.publicKey) {
            // get public key
            const publicKeyBase58Decoded = this.getBytesPublicKey(
                signer.publicKey
            );

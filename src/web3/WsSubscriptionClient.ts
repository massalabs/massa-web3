...
/**
 * Converts any union of `ArrayBuffer`|`Buffer`|`Buffer[]` to a string
 * 
 * @returns the reduced to string arraybuffer-like input
 */
const arrayBufferToBase64 = (array: Buffer | ArrayBuffer | Buffer[]): string => {
...
export class WsSubscriptionClient extends WsBaseClient {
	...
	/**
   * Constructs a new WsSubscriptionClient with a config
   *
   * @param clientConfig - Client Configuration Object.
   * @returns `WsSubscriptionClient` Instance
   */
	...
	public subscribeNewBlockHeaders(onNewBlockHeaderHandler?: (data: IBlockHeaderInfo) => void): void {
	...
	public unsubscribeNewBlockHeaders(): void {
	  ...
	  this.wss.send(JSON.stringify({
	    ...
	    "params": [this.subMethods.get(WS_RPC_REQUEST_METHOD_NAME.NEW_BLOCKS_HEADERS)]
	  }));
	}
	...
	public unsubscribeFilledBlocks(): void {
	  ...
	  this.wss.send(JSON.stringify({
	    ...
	    "params": [this.subMethods.get(WS_RPC_REQUEST_METHOD_NAME.NEW_FILLED_BLOCKS)]
	  }));
	}
	    ...
	    if (parsedMsg["params"] && parsedMsg["params"]["result"] && parsedMsg["params"]["subscription"]) {
	      ...
	      switch (matchMethodName(parsedMsg["method"])) {
	        case WS_RPC_REQUEST_METHOD_NAME.NEW_BLOCKS: {
	          if (this.subIds.has(subId) && this.subIds.get(subId) !== WS_RPC_REQUEST_METHOD_NAME.NEW_BLOCKS) {
	        ...
	        case WS_RPC_REQUEST_METHOD_NAME.NEW_BLOCKS_HEADERS: {
	          if (this.subIds.has(subId) && this.subIds.get(subId) !== WS_RPC_REQUEST_METHOD_NAME.NEW_BLOCKS_HEADERS) {
	            this.subIds.set(subId, matchMethodName(parsedMsg["method"]));
	        ...
	        case WS_RPC_REQUEST_METHOD_NAME.NEW_FILLED_BLOCKS: {
	          if (this.subIds.has(subId) && this.subIds.get(subId) !== WS_RPC_REQUEST_METHOD_NAME.NEW_FILLED_BLOCKS) {
	            this.subIds.set(subId, matchMethodName(parsedMsg["method"]));

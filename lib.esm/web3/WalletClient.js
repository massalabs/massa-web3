...
export class WalletClient extends BaseClient {
    ...
    async addSecretKeysToWallet(secretKeys) {
        ...
        for (const secretKey of secretKeys) {
            ...
            const publicKeyBase58Encoded = PUBLIC_KEY_PREFIX + base58Encode(Buffer.concat([version, publicKey]));
    ...
    static async walletGenerateNewAccount() {
        ...
        const version = Buffer.from(varintEncode(VERSION_NUMBER));
        const secretKeyBase58Encoded = SECRET_KEY_PREFIX + base58Encode(Buffer.concat([version, secretKey]));
        // get public key
    ...
    static getBytesSecretKey(secretKey) {
        ...
        const secretKeyVersionBase58Decoded = base58Decode(secretKey.slice(1));
        // Version is little for now
    ...
    async sendTransaction(txData, executor) {
        ...
        // bytes compaction
        const bytesCompact = this.compactBytesForOperation(txData, OperationTypeId.Transaction, sender, expiryPeriod);
        // sign payload
    ...
    async buyRolls(txData, executor) {
        ...
        const expiryPeriod = nodeStatusInfo.next_slot.period + this.clientConfig.periodOffset;
        // bytes compaction
        const bytesCompact = this.compactBytesForOperation(txData, OperationTypeId.RollBuy, sender, expiryPeriod);
        // sign payload
        const signature = await WalletClient.walletSignMessage(Buffer.concat([
            WalletClient.getBytesPublicKey(sender.publicKey),
            bytesCompact,
    ...
    async sellRolls(txData, executor) {
        ...
        const expiryPeriod = nodeStatusInfo.next_slot.period + this.clientConfig.periodOffset;
        // bytes compaction
        const bytesCompact = this.compactBytesForOperation(txData, OperationTypeId.RollSell, sender, expiryPeriod);
        // sign payload
        const signature = await WalletClient.walletSignMessage(Buffer.concat([
            WalletClient.getBytesPublicKey(sender.publicKey),
            bytesCompact,

...
export class SmartContractsClient extends BaseClient {
    ...
    async deploySmartContract(contractData, executor) {
        ...
        // bytes compaction
        const bytesCompact = this.compactBytesForOperation(contractData, OperationTypeId.ExecuteSC, sender, expiryPeriod);
        // sign payload
    ...
    async callSmartContract(callData, executor) {
        ...
        // bytes compaction
        const bytesCompact = this.compactBytesForOperation(callData, OperationTypeId.CallSC, sender, expiryPeriod);
        // sign payload
        const signature = await WalletClient.walletSignMessage(Buffer.concat([
            WalletClient.getBytesPublicKey(sender.publicKey),
            bytesCompact,

...
const bs58EncodeCheck = (data, prefix = '00', encoding = 'hex') => {
    ...
    const hash = (0, crypto_1.createHash)('sha256').update(buffer_1.Buffer.concat([_prefix, data])).digest();
    const checksum = (0, crypto_1.createHash)('sha256').update(hash).digest().slice(0, 4);
...
const bs58DecodeCheck = (string, encoding) => {
    ...
    const hash = (0, crypto_1.createHash)('sha256').update(buffer_1.Buffer.concat([prefix, data])).digest();
    const calculatedChecksum = (0, crypto_1.createHash)('sha256').update(hash).digest().slice(0, 4);

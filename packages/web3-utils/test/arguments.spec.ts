...
describe('Args class', () => {
  ...
  it('should correctly serialize and deserialize an Args object containing a u128, i128, an u256 and a string', () => {
    const u128Val = 146738984765738234n;
    const i128Val = -146738984765738234n;
    const u256Val = 146738984765738234146738984765738234n;
    const str = 'random string lolmao';
    const args1 = new Args();
    args1.addU128(u128Val);
    args1.addI128(i128Val);
    args1.addU256(u256Val);
    args1.addString(str);

    const args2 = new Args(args1.serialize());
    expect(args2.nextU128()).toEqual(u128Val);
    expect(args2.nextI128()).toEqual(i128Val);
